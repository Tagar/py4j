...
**Target branch:** `perf-framework` on `github.com/Tagar/py4j`
...
Brainstormed 2026-04-19 in conversation with Ruslan Dautkhanov. Seven design questions resolved:

1. Audience → **A** (local tool).
2. Framework → **C** (hybrid).
3. Scenarios → **A** (all 13).
4. JVM lifecycle → auto-spawn + fresh per scenario.
5. Output → markdown + JSON, `--compare`, verdicts + noise bands, environment header.
6. Statistical rigor → pytest-benchmark defaults + custom runner w/ gc disable; fixed-heap JVM; env guards with `--strict`.
7. Layout / deps → `py4j/tests/perf/` module, `pytest-benchmark` + `psutil`, auto-spawn, keep `benchmark1.py` with deprecation comment for now.

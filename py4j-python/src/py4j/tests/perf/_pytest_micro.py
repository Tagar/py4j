...
from py4j.tests.perf.report import build_scenario_entry
...
def _translate(bench_json_path):
    ...
    for entry in data.get("benchmarks", []):
        ...
        # Route through build_scenario_entry so micro scenarios pick up
        # the same derived-metrics block (throughput, latency_per_op,
        # bandwidth, cpu_time_ratio, errors) and noise-budget fields as
        # macro scenarios. pytest-benchmark doesn't expose per-round
        # CPU time, so cpu_time_ratio stays None for micros — that's
